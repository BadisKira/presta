...
public class ContractorRepositoryUseCase implements ContractorRepositoryPort {
    ...
    public boolean isActive(UUID contractorId) {
        Contractor existingContractor = userRepositoryPort.findContractorById(contractorId)
                .orElseThrow(() -> new IllegalArgumentException("Contractor non trouvé avec l'ID: " + contractorId));
        return existingContractor.user().isActive();
    }
